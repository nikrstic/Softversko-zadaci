...
public class Graf {
	...
	public void dodajGranuUsmereni(int pocetak, int kraj,int tezina)
	{
		if(!postojiGrana(pocetak,kraj)) {
			Edge edge = new Edge(pocetak, kraj, tezina);
			edges.add(edge);
			nodes.get(pocetak).add(edges.size() - 1);
		}
	}
	public boolean postojiGrana(int pocetak, int kraj){
		boolean postoji=false;
		for(int i=0;i<edges.size();i++){
			Edge e = edges.get(i);
			if(e.pocetak==pocetak && e.kraj == kraj){
				postoji=true;
			}
		}
		return postoji;
	}
	public void DodajGranuNeusmereni(int pocetak, int kraj, int tezina){
		if(!postojiGrana(pocetak,kraj)) {
			Edge edge = new Edge(pocetak, kraj, tezina);
			edges.add(edge);
			nodes.get(pocetak).add(edges.size()-1);
		}
		if(!postojiGrana(kraj,pocetak)) {
			Edge edge = new Edge( kraj, pocetak,tezina);
			edges.add(edge);
			nodes.get(kraj).add(edges.size()-1);
		}
	}
	//Komponente povezanosti
	public Vector<Vector<Integer>> KomponentePovezanosti(){
		...
		for(int i=1;i<=numNodes;i++){
			...
			if(!nadjen.get(i)){
				Vector<Integer> komponenta = new Vector<Integer>();
				DfsPom(i,komponenta,nadjen);
				result.add(komponenta);
			}
	...
	public int rastojanje(int u, int v){
		Vector<Integer> rastojanja= Dijkstra(u);
		//System.out.println(rastojanja.toString());
		return rastojanja.get(v-1);
	}
	//DFS rekurzivno
